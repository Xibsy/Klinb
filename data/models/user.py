...
class User(SqlAlchemyBase):
    ...
    show_aaa = sql.Column(sql.Boolean, default=False)
    bio = sql.Column(sql.String(500))
    ...
    def create_user(cls, name: str, username: str, password: str, avatar: str = None, discord: str = None,
                    ...
                    incoming_requests: str = None, outgoing_requests: str = None, status: str = None,
                    show_aaa: bool = False, bio: str = None) -> "User":
        ...
        user = cls(name=name, username=username, avatar=avatar or '/static/uploads/volosatic.jpg', discord=discord,
                   ...
                   outgoing_requests=outgoing_requests, status=status, show_aaa=show_aaa, bio=bio)
    ...
    def to_dict(self) -> dict[str, Column[str] | Column[int]]:
        return {'id': self.id, 'name': self.name, 'username': self.username, 'avatar': self.avatar,
                ...
                'outgoing_requests': self.outgoing_requests, 'status': self.status,
                'show_aaa': self.show_aaa, "bio": self.bio}